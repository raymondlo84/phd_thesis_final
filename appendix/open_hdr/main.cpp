...
float basis [16] = {-1.0f,3.0f,-3.0f,1.0f,
    3.0f,-6.0f,3.0f,0.0f,
    -3.0f,0.0f,3.0f,0.0f,
    1.0f,4.0f,1.0f,0.0f};
...
CvScalar rgb[3] = {cvScalar(255,0,0,0),cvScalar(0,255,0,0),cvScalar(0,0,255,0)};
        ...
        for (l=0;l<xPts/xInc;l++){
            for (k=0;k<ptsInterpolate;k++){
                ...
                for (i=0;i<4;i++){
                    ...
                    for(j=0;j<4;j++){
                        xIndex = (l-1+j)*xInc;
                        if(xIndex < 0)	tmpX = x[m];
                        else if (xIndex >= xPts) tmpX = x[(xPts-1)*n+m]*1.1f;
                        else tmpX = x[xIndex*n+m];
...
void sigmoid_dist(float* x, int xPts, int n){
    ...
    for(j=0;j<n;j++){
        for(i=0;i<xPts;i++){
            x[i*n+j] = sigmoid((float)(i-(xPts/2-1))/(float)(xPts/8-1),1.0f,0.0f);
        }
    }
}
...
float q_comparagram(unsigned char* src0, unsigned char* src1, unsigned int img_size, float* inverse_response, float* certainty){
...
void compute_weight(float* x, unsigned int xPts, float* tmp_x, unsigned int n, unsigned int __a, unsigned int __b, unsigned int __c, unsigned int __total){
...
int main(int argc, char *argv[]){
    ...
    unsigned int *histogram = (unsigned int*)malloc(sizeof(unsigned int)*histogramSize);
    ...
    compute_weight(sample_weight, PIX_DEP, tmp_weight, num_img, _a, _b, _c, _total);
    ...
    for(int i = 0; (stop_detected == 0) || (i < 50*num_img);i++){
        ...
        if(downSize == 0){
            if(i > 0 && stop_detected == 0)
                pair_stop += q_comparagram((unsigned char*)frame->imageData,(unsigned char*)prev_frame->imageData, wid*hi*3, response_CPU, sample_weight);
            ...
            if(i > 0 && stop_detected == 0)
                pair_stop += q_comparagram((unsigned char*)qFrame->imageData,(unsigned char*)prev_frame->imageData, wid*hi*3, response_CPU, sample_weight);
    ...
    while((frame = cvQueryFrame(capture)) != NULL){
        ...
        max_index = useLIGHT((float)(_toneP-200)/10.0f, (float)num_img/2.0f);
